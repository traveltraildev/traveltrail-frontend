...
const AccommodationDetailsPage = ({ isMobile }) => {
  const { id } = useParams();
  const accommodation = accommodationsData.find(acc => String(acc.id) === String(id));
  ...
  return (
    <>
      ...
      <Container maxWidth="lg" sx={{ paddingTop: "20px", paddingBottom: "20px", marginTop: "60px" }}>
                ...
                <Box sx={{ padding: "20px" }}>
                  ...
                  <Card elevation={2} sx={{ mb: 2, borderRadius: "15px" }}>
                    <CardContent sx={{ padding: "16px" }}>
                      ...
                      <Typography variant="body1" sx={{ color: "text.secondary" }}>{accommodation?.overview}</Typography>
                    </CardContent>
                  </Card>
                  ...
                  <Card elevation={2} sx={{ mb: 2, borderRadius: "15px" }}>
                    <CardContent sx={{ padding: "16px" }}>
                      <Typography variant="h6" sx={{ fontWeight: "bold", mb: 1 }}>Themes</Typography>
                      <Typography variant="body1" sx={{ color: "text.secondary" }}>{accommodation?.themes?.map(theme => " " + theme)}</Typography>
                    </CardContent>
                  </Card>
                  ...
                  <Card elevation={2} sx={{ mb: 2, borderRadius: "15px" }}>
                    <CardContent sx={{ padding: "16px" }}>
                      <Typography variant="h6" sx={{ fontWeight: "bold", mb: 1 }}>Inclusions</Typography>
                      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "repeat(auto-fit, minmax(100px, 1fr))" }, gap: "8px" }}>
                        {accommodation?.inclusions?.map((inclusion, index) => (
                          <Typography key={index} sx={{ backgroundColor: "skyblue", padding: "4px 8px", borderRadius: "8px", fontSize: "0.875rem", textAlign: "center" }}>{inclusion}</Typography>
                        ))}
                      </Box>
                    </CardContent>
                  </Card>
                  ...
                  <Card elevation={2} sx={{ borderRadius: "15px" }}>
                    <CardContent sx={{ padding: "16px" }}>
                      <Typography variant="h6" sx={{ fontWeight: "bold", mb: 1 }}>Exclusions</Typography>
                      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "repeat(auto-fit, minmax(100px, 1fr))" }, gap: "8px" }}>
                        {accommodation?.exclusions?.map((exclusion, index) => (
                          <Typography key={index} sx={{ backgroundColor: "skyblue", padding: "4px 8px", borderRadius: "8px", fontSize: "0.875rem", textAlign: "center" }}>{exclusion}</Typography>
                        ))}
                      </Box>
                    </CardContent>
                  </Card>
