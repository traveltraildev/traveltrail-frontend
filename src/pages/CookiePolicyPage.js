      ...
      try {
        ...
        if (!response.ok) {
           if (response.status === 404) {
            setPageContent({ title: "Cookie Policy", content: "<p>The content for this page is not available yet.</p>" });
           } else {
   ...
   if (!pageContent) {
     return (
        <Box sx={{ textAlign: 'center', py: 12 }}>
          <Typography variant="h4">Failed to load Cookie Policy content.</Typography>
        </Box>
      );
   }
